...
module Holidays::FunctionProcessor
  class << self
    def process_function(input, func_id, desired_func_args, func_modifier = nil)
      validate!(input, func_id, desired_func_args)

      function = Holidays::Factory::Definition.custom_methods_repository.find(func_id)
    ...
    def calculate(input, id, args, modifier)
      result = Holidays::Factory::Definition.proc_result_cache_repository.lookup(id, *args)
